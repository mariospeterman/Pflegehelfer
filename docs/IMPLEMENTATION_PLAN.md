...
5. Terminal answers select exact run-local claims, and deterministic server code renders record facts as same-row clinical atoms rather than trusting model prose or German keyword overlap. Harmless source-free replies are server-owned. Durable source IDs, versions, hashes and provenance never cross the model boundary, while archived evidence binds exact claims to the complete server-side result digest. Proposal revisions retain immutable hashed input records, the Luca 200→150 ml chain revalidates all contributing turns, unknown corrections clarify instead of silently preserving stale meaning, and no-model mode is explicit with verbatim review.
