...
Next vertical slice remains G1, not a new fork: migrate the now fail-closed direct detail mutation routes to the accepted-command boundary, restore current clinical state from scoped FHIR resources rather than the whole-state Binary, add relational inbound/conflict reconciliation and project command/manual states into the normal GenUI. The old checkpoint processor is disabled in the integrated profile and accepted assistant commands remove their transient legacy queue copies after relational acceptance; do not reconnect it in parallel. Then proceed to G3 shared-thread ACLs and append-only G2 handover addenda.
