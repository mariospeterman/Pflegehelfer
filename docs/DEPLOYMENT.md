...
Legacy assistant transcripts are quarantined and never read or relabelled. If an active session in the exact configured institution and department still points at one, startup stops unless an operator has explicitly reviewed that department and temporarily sets `PFH_ALLOW_LEGACY_THREAD_QUARANTINE_RELINK=true`. The migration creates a fresh site-scoped general thread and relinks the existing session so its work episodes and responsibility remain intact; the ambiguous transcript stays expired and all linked authorities are revoked. Remove the one-time flag immediately after the recorded migration.
