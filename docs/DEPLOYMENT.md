...
The Compose profile is the complete fictional showcase. The Helm chart is an application baseline, not a claim that a healthcare production platform is configured.
...
The current modular-monolith repository is deliberately single-replica: the Helm default is one because its coherent read model is process-local. Conditional checkpoint writes protect against accidental stale writers but are not a multi-replica read-consistency design. Do not scale the application above one replica until a shared transactional repository or per-read checkpoint refresh is implemented and load-tested.
