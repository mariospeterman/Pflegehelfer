...
Organization economics reuse the same institution boundary and operational PostgreSQL. `organization_commercial_config` holds optimistic-versioned, effective-dated pilot/deployment/organization-subscription/pooled-usage configuration; `organization_usage_ledger` deduplicates provider receipts. Server code calculates integer-minor-unit review-draft statements, BYO-key no-double-charge, local-inference exclusions, pooled allowance, alerts and limits. There is deliberately no employee/seat/device/role charging entity and no payment collector. Commercial state never widens clinical permissions. A measured exhausted limit is consulted only before a new model request; manual work and already-accepted projection/provider jobs do not depend on it.
...
| Organization economics | Pflegehelfer PostgreSQL | Contract scope and idempotent provider usage; never a clinical authorization source |
