...
4. Retain successful tool results in a request-local immutable evidence registry independent of rendered components. Natural text is the default. The model may optionally choose a table or chart from the exact presentation catalog; deterministic code resolves its fields from one cited result and hydrates only registered server components. Invented/missing references, unsupported claims, executable links/tokens and claims of completed writes fail closed. Generated prose remains assistant output, not a signed clinical fact. Tool results are marked untrusted data and remain thread/patient/encounter scoped.
