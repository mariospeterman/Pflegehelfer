...
Every executable intent is one-use, short-lived, stored as a cryptographic hash, and bound to organization, actor, role, purpose, patient/encounter, session/thread, workflow version/step, context revision, proposal hash, policy version and resource versions. The production target consumes authority and accepts the complete local command in one PostgreSQL transaction. The current runtime has durable one-use authority but has not yet unified its checkpoint mutation, FHIR outbox and relational provider job into that transaction; this remains a production blocker.
...
Workers claim rows with bounded leases and `FOR UPDATE SKIP LOCKED`, stable idempotency keys, classified errors, exponential backoff and terminal/manual poison state. Every leased command must carry its immutable acceptance-time actor/purpose/policy/authority-revision envelope and pass delivery-time authorization; revoked or unverifiable authority enters manual hold before an adapter is called. A crashed lease expires safely. Acknowledgements/conflicts are later transactions. The isolated kernel already requires the authorization callback, but it remains unwired until the durable envelope and atomic local acceptance exist.
